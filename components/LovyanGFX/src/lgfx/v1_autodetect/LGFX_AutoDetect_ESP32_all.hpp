    ...
    // ESP_LOGI("DEBUG","AXP0x03 :%02x ", lgfx::i2c::readRegister8(i2c_port, axp_i2c_addr, 0x03, 400000).value());
      ...
      struct _detector_ESP32_S3_BOX_V3_t : public _detector_spi_t
      {
        ...
        void setup(_detector_result_t* result) const override
        {
          ...
          // Touch_GT911 disabled. The Box-3 V3's GT911 shares I2C_NUM_0
          // (SDA=8, SCL=18) with the ES8311 audio codec, and IDF v5.x's
          // new i2c_master driver can't have two clients installing their
          // own bus on the same port. Wire (used by audio.cpp) would
          // fail with ESP_ERR_INVALID_STATE the moment LovyanGFX claimed
          // the port for touch. Box-3 V3's touch panel was also failing
          // to ACK at boot ("i2c read error : read timeout"), so we're
          // not losing functionality the simulator uses. If touch is
          // ever needed, the right fix is to share a single bus handle
          // between LovyanGFX and audio.cpp rather than re-enable this.
        }
      };
