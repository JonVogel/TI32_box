...
// The Arduino IDE auto-inserts these when compiling a .ino, but plain
// C++ (ESP-IDF) doesn't, so we list the ones referenced out of order.
// ---------------------------------------------------------------------------
struct LineEdit;
...
extern int  g_catLines;
extern bool g_catCancelled;

// ---------------------------------------------------------------------------
// Tokenizer + detokenizer live in host_common now (adopted sunton's newer
// version with TOK_LINENUM after GOTO/GOSUB/THEN + IMAGE literal capture +
// `!` tail comments + unquoted OLD DSK1.NAME file-specs).
// See components/ti-emulator/components/host_common/src/ti_host_tokenizer.cpp.
...
//
// The interpreter, file I/O, and any other long-running BASIC code path
// calls this so the FreeRTOS scheduler can run lower-priority tasks —
// in particular the per-CPU IDLE task. delay(1) puts the calling task
// into the Blocked state for one FreeRTOS tick (~1 ms at HZ=1000),
// which is enough for IDLE1 to run, do its housekeeping, and reset its
// task-watchdog subscription. Arduino's bare yield() / FreeRTOS's
// taskYIELD() only switch among same-or-higher-priority tasks and
// would NOT let IDLE run.
void tiYield()
{
...
// implementation, then sync the box-local fgColor/bgColor cache
// (used by initDisplay() and some direct tft calls). host_common's
// version handles charFgIdx/charBgIdx/screenColorIdx uniformly across
// hosts; the fgColor/bgColor mirror is a box-specific optimization.
...
// inputBuf / inputPos / inputReady are the editor→main-loop handoff
// interface. Moved to host_common along with the editor loop; using
// them here just re-imports the names for main.cpp's if (inputReady)
// { processInput(inputBuf); ... } check.
...
// state all moved to host_common (ti_host_editor.cpp). processEditChar
// is shared too now. getInputLine / checkInput / editorBeginLine
// still live in this file because they touch box's BLE keyboard
// driver and audio codec.
...
// Paste ring + editorReadChar + editBufferIsAllDigits + editorBuffer-
// IsAutoFillOnly moved to host_common. The interpreter-integration
// helpers below (findProgramLineIndex / commitEditedLine /
// loadProgramLineToEdit) are box-local implementations that touch em
// (the ExecManager instance) + the tokenizer/detokenizer; they get
// registered with host_common via setInterpreterHooks() in setup()
// so the shared processEditChar can call through them.
...
// hostReadBleKey wrapper — combined "have BLE key? read it" — used by
// host_common's editorReadChar via the TiDisplay hook.
...
// TiInterpreterHooks implementations. These stay per-host because they
// call em (the local ExecManager instance) + tokenizeLine +
// detokenizeLine directly; wiring them into host_common as callbacks
// keeps the interpreter instance out of shared code for now.
...
// all moved to host_common — see `using` block near line 1147.
...
// / programSize), so nothing box-specific remains in the keybind
// dispatch.

// getInputLine / checkInput / editorBeginLine / editorCursorTick all
// moved to host_common — they call editorReadChar (drains paste ring +
// dispatches to hostReadBleKey) + processEditChar via shared code,
// and pump BLE via display.hostKbTask on hosts that provide it.
...
{
  char buf[256];
  // Route through catPrintLine so LIST also paginates every 23 lines
  // (matches TI behavior + DIR/CAT) and yields per line.
  g_catLines = 0;
  g_catCancelled = false;
  for (int i = 0; i < em.programSize() && !g_catCancelled; i++)
  {
...
int  g_catLines     = 0;
bool g_catCancelled = false;
static void catPrintLine(const char* s)
{
  if (g_catCancelled) return;

  // Count SCREEN rows this line will occupy — a long BASIC line wraps
  // across multiple 32-char display rows. Empty line still takes 1.
  int len = s ? (int)strlen(s) : 0;
  int rows = (len + COLS - 1) / COLS;
  if (rows < 1) rows = 1;

  // If printing this line would push us past the 23-row content area
  // (row 24 is reserved for the prompt), page BEFORE we print — so the
  // user always sees full content before the prompt and we never spill
  // a partial line off the top.
  if (g_catLines + rows > 23)
  {
  ...
  printLine(s);
  g_catLines += rows;
  // Per-line yield (DIR on a big SD card can be 100+ entries — don't
  // wait 23 rows to let the scheduler run).
  tiYield();
}
...
void tiWifiOn()
{
  ...
  g_wifiPrefs.end();
  // Restore the friendly hostname BEFORE webfiles::init() runs so the
...
{
  // Yield first so we yield once per BASIC line (this is registered as
  // the interpreter's per-line tick callback in setup). Without it, a
  // tight BASIC numeric loop never gives FreeRTOS a slot — the
  // interpreter's own tiYield call at the end of each line still helps,
  // but doing it here too is cheap and covers the sprite-demo case
  // (movement looks the same when the scheduler can't catch up).
  tiYield();
...
void setup()
{
  ...
  // filled in here on top of what init() set.
  {
    ...
    // Populate the token -> text lookup table used by detokenizeLine.
    tihost::initTokenNames();

    // Wire the interpreter callbacks host_common's editor calls when
    // it needs to look up a program line or commit an edit. The impls
    // above pass through to box's local em (ExecManager instance).
...
void loop()
{
  ...
  // Yield once per main-task pass so the FreeRTOS scheduler can run
  // IDLE1, the BLE host task, etc. tiYield() is the host's strong
  // override of the interpreter's weak hook — same yield primitive
  // used everywhere in the interpreter and graphics layers.
  tiYield();
}
